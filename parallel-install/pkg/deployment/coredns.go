...
func patchCoreDNS(kubeClient kubernetes.Interface, overrides *OverridesBuilder, isK3s bool, log logger.Interface) (cm *v1.ConfigMap, err error) {
	err = retry.Do(func() error {
		...
		patches, err := generatePatches(kubeClient, overrides, isK3s)
		if err != nil {
			return err
		}
		if len(patches) != 0 {
			cm, err = doPatch(kubeClient, patches, log)
			return err
		}
		return nil
	}, defaultRetryOptions()...)
...
func doPatch(kubeClient kubernetes.Interface, patches map[string]string, log logger.Interface) (cm *v1.ConfigMap, err error) {
	...
	if exists {
		...
		return patchCoreDNSConfigMap(configMaps, coreDNSConfigMap, patches, log)
	} else {
		log.Info("Corefile not found, creating new CoreDNS config")
		return createCoreDNSConfigMap(configMaps, patches, log)
	}
}
