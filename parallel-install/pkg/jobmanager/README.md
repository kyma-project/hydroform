...
Furthermore, the `jobManager` package has a `duration` variable for benchmarking, which will be fetched at the end of a Kyma deployment.
...
At this point, we will stick to the passiv solution, we will may switch to the active solution in the future. If no deprecation version is decided or not necessary, should also be documented.
